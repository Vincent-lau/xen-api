  ...
  let copy_into_vdi ~task ~dbg ~sr ~vdi ~vm ~url ~dest ~dest_vdi ~verify_dest =
    ...
    let dest_vdi_url =
      ...
      Http.Url.set_uri url'
        (Printf.sprintf "%s/nbdproxy/import/%s/%s/%s/%s" (Http.Url.get_uri url')
    ...
    let id = Storage_migrate_helper.State.copy_id_of (sr, vdi) in
    ...
    SXM.info "%s: copy initiated local_vdi:%s dest_vdi:%s" __FUNCTION__
      ...
      (fun () ->
        ...
        let backend = Remote.VDI.attach3 dbg remote_dp dest dest_vdi vm true in
        let proto =
          nbd_export_of_attach_info backend
          |> Option.map (fun export -> StreamCommon.Nbd export)
        in
...
let mirror_pass_fds ~dbg ~dp ~sr ~vdi ~mirror_vm ~live_vm ~mirror_id ~url
  ...
  debug "%s url of http request for mirroring is %s" __FUNCTION__
  ...
  let tapdev =
    ...
    | Some tapdev ->
        ...
        tapdev
    | None ->
        error "%s: vdi %s not attached" __FUNCTION__ (Vdi.string_of vdi) ;
        raise
          (Storage_interface.Storage_error
             (Migration_mirror_fd_failure "VDI Not Attached")
          )
    | exception e ->
        error "%s Caught exception %s:. Performing cleanup." __FUNCTION__
          (Printexc.to_string e) ;
        raise
          (Storage_interface.Storage_error
...
module MIRROR : SMAPIv2_MIRROR = struct
  ...
  let receive_start_common _ctx ~dbg ~sr ~vdi_info ~id ~similar ~vm
      (module SMAPI : SMAPIv2) =
    info
      "DATA.MIRROR.receive_start_common dbg:%s sr:%s id:%s similar:[%s] vm:%s"
      dbg (s_of_sr sr) id
      (String.concat "," similar)
      (s_of_vm vm) ;
    let on_fail : (unit -> unit) list ref = ref [] in
    let vdis, _ = SMAPI.SR.scan2 dbg sr in
    (* We drop cbt_metadata VDIs that do not have any actual data *)
  ...
  let receive_start ctx ~dbg ~sr ~vdi_info ~id ~similar =
    let remote_mirror =
      receive_start_common ctx ~dbg ~sr ~vdi_info ~id ~similar
        ~vm:(Vm.of_string "0")
        (module Local)
    in

    debug "%s updating receiving state to id: %s vm: %s vdi_info: %s"
      __FUNCTION__ id "0"
      (string_of_vdi_info vdi_info) ;
    State.update_mirror_recv_state ~sr ~vdi_info ~mirror_id:id
      ~vm:(Vm.of_string "0") ~url:"" ~verify_dest:false remote_mirror ;
    remote_mirror
  ...
  let receive_start2 ctx ~dbg ~sr ~vdi_info ~mirror_id ~similar ~vm ~url
    ...
    in
    let remote_mirror =
      receive_start_common ctx ~dbg ~sr ~vdi_info ~id:mirror_id ~similar ~vm
        (module Remote)
    in
    debug "%s updating receiving state to id: %s vm: %s vdi_info: %s"
  ...
  let receive_finalize2 _ctx ~dbg ~mirror_id ~sr ~url ~verify_dest =
    debug "%s dbg: %s id: %s sr: %s url: %s verify_dest: %B" __MODULE__ dbg
      mirror_id (Sr.string_of sr) url verify_dest ;
    ...
    Option.iter
      (fun r ->
        ...
        Option.iter
          (fun parent_vdi ->
            SXM.info
              ...
              __FUNCTION__ (Sr.string_of r.sr) (Vdi.string_of parent_vdi)
              (Vdi.string_of r.leaf_vdi) ;
            Remote.VDI.compose dbg r.sr parent_vdi r.leaf_vdi
          )
          r.parent_vdi ;
        ...
        log_and_ignore_exn (fun () ->
            Option.iter
              (fun dummy_vdi -> Remote.VDI.destroy dbg r.sr dummy_vdi)
              r.dummy_vdi
        ) ;
  ...
  let receive_cancel2 _ctx ~dbg:_ ~id:_ ~url:_ ~verify_dest:_ =
