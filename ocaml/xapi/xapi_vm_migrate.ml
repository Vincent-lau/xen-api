    ...
    VDIMap.iter
      (fun _ (mirror, snapshots) ->
        ...
        Storage_migrate.update_snapshot_info_src ~dbg ~sr ~vdi ~url ~dest
          ~dest_vdi ~snapshot_pairs ~verify_dest
      )
  ...
  let mirror_to_remote new_dp =
    let task =
      if not vconf.do_mirror then
        Storage_migrate.copy ~dbg ~sr:vconf.sr ~vdi:vconf.location
          ~vm:vconf.copy_vm ~url:remote.sm_url ~dest:dest_sr
        ...
        (* SMAPI.VDI.activate3 dbg new_dp vconf.sr vconf.location live_vm ; *)
        let live_vm =
        ...
        (* Is there a need for this to be multiplexed, we can mux inside MIRROR.start *)
