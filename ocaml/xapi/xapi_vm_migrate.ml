...
module SXM = Debug.Make (struct let name = "SXM" end)
...
let assert_sr_support_operations ~__context ~vdi_map ~remote ~ops =
  let op_supported_on_source_sr vdi ops =
    ...
    then (
      debug "src sm features: %s, ops: %s"
        (List.map fst sr_features
        |> List.map Smint.string_of_capability
        |> String.concat ";"
        )
        (List.map Smint.string_of_capability ops |> String.concat ";") ;
      raise
  ...
  let op_supported_on_dest_sr sr ops sm_record remote =
    ...
    then (
      debug "dest sm caps: %s, ops: %s"
        (String.concat ";" sm_capabilities)
        (List.map Smint.string_of_capability ops |> String.concat ";") ;
      raise
        (Api_errors.Server_error
...
let get_vdi_mirror __context vm vdi do_mirror =
  ...
  in
  let copy_vm =
    Ref.string_of vm |> ( ^ ) "SXM-copy-" |> Storage_interface.Vm.of_string
  in
  let mirror_vm =
    Ref.string_of vm |> ( ^ ) "SXM-mirror-" |> Storage_interface.Vm.of_string
  in
  {
    vdi
  ; dp
...
let vdi_copy_fun __context dbg vdi_map remote is_intra_pool remote_vdis so_far
  ...
  let with_remote_vdi remote_vdi cont =
    ...
    debug "Got remote vdi str %s" remote_vdi_str ;
  ...
  let post_mirror mirror_id mirror_record =
    try
      ...
      result
    with e ->
      debug "%s: error %s" __FUNCTION__ (Printexc.to_string e) ;
      let mirror_failed =
        match mirror_id with
        | Some mid ->
            ignore (Storage_access.unregister_mirror mid) ;
            debug "%s stat now" __FUNCTION__ ;
  ...
  if mirror then
    with_new_dp (fun new_dp ->
        ...
        with_remote_vdi remote_vdi (fun remote_vdi_ref ->
            debug "%s pre getting mirror record" __FUNCTION__ ;
            let mirror_record =
              get_mirror_record ~new_dp remote_vdi remote_vdi_ref
            in
            debug "%s post getting mirror record" __FUNCTION__ ;
...
let assert_can_migrate ~__context ~vm ~dest ~live:_ ~vdi_map ~vif_map ~options
  ...
  let required_sr_operations = [Smint.Vdi_snapshot] in
