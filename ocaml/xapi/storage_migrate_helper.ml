module D = Debug.Make (struct let name = "storage_migrate_helper" end)
...
module State = struct
  module Receive_state = struct
    type t = {
        sr: Sr.t
      ; dummy_vdi: Vdi.t option
      ; leaf_vdi: Vdi.t
      ; leaf_dp: dp
      ; parent_vdi: Vdi.t option
      ; remote_vdi: Vdi.t
      ; mirror_vm: Vm.t
      ; url: string [@default ""]
  ...
  module Send_state = struct
    ...
    type t = {
      ...
      ; live_vm: Vm.t
            [@default Vm.of_string "0"] (* vm source vdi is attached to *)
  ...
  let update_mirror_recv_state ~sr ~vdi_info ~mirror_id ~vm ~url ~verify_dest =
    function
    | Storage_interface.Mirror.Vhd_mirror vhd_res ->
        add mirror_id
          (Recv_op
             Receive_state.
               {
                 sr
               ; dummy_vdi= Some vhd_res.dummy_vdi
               ; leaf_vdi= vhd_res.mirror_vdi.vdi
               ; leaf_dp= vhd_res.mirror_datapath
               ; parent_vdi= Some vhd_res.copy_diffs_to
